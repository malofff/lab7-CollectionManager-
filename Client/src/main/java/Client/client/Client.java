...
public class Client implements ClientApp {
    ...
    public Client(ClientCommandReaderImpl commandReader,
        ...
        this.port = port;
        humanBeingBuilder = new HumanBeingBuilderImpl(getReader(), false, new HumanBeingValidatorImpl());
        isRunning = true;
        addCommands();
    }
    ...
    public void start(int port) {
        while (isRunning) {
            ...
            try {
                ...
                } catch (IOException | ClassNotFoundException | InvalidFieldException ioException) {
     ...
     */
    public Response communicateWithServer(String inputString) throws IOException, ClassNotFoundException, InvalidFieldException {
        ...
        if (response == null) {
            ...
            return null;
        } else if (response.getResponseType().equals(ResponseType.HUMAN_BEING_RESPONSE)) {
            request.setUser(currentUser);
     ...
     * @return Response - ответ сервера
     */
    private Response reCommunicateWithServer(Request request) throws IOException, ClassNotFoundException, InvalidFieldException {
        String arg = request.getArg();
        SocketChannel socketChannel = connectionManager.openConnection(port);
        HumanBeing humanBeing = humanBeingBuilder.askHumanBeing();
        humanBeing.setUsername(currentUser.getUserName());
        request = requestSender.createExecuteRequest(request.getCommandName(), humanBeing);
        request.setArg(arg);
